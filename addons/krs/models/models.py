...
class krs_otel(models.Model):
	...
	name = fields.Char(string=u"Наименование", required=True, compute='_raschet', store=True)
	inv_nomer = fields.Char(string=u"Инв. №", required=True)
	kod_hoz = fields.Integer(string=u"Код хоз. рождения",required=True, store=True)
	date = fields.Date(string='Дата', required=True, default=fields.Datetime.now)
	nomer_lakt = fields.Integer(string=u"Номер лактации",required=True, store=True)
	result = fields.Char(string=u"Результат", required=True)
	krs_result_otel_id = fields.Many2one('krs.result_otel', string='Результат отела', required=True)
	krs_hoz_id = fields.Many2one('krs.hoz', string='Хозяйство рождения', required=True)
